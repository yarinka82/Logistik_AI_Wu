...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            self.stdout.write("Створюю компанію-перевізника зі штатом і автопарком...")
            carrier = self.create_carrier_with_fleet()

            self.stdout.write("Створюю водія-одноосібника...")
            self.create_solo_driver()

            self.stdout.write("Створюю бухгалтера...")
            self.create_accountant(carrier)
    ...
    def create_client_individuals(self):
        ...
        for email, username, full_name in people:
            user, created = self._get_or_create_user(email, username, User.Role.CLIENT_INDIVIDUAL)
            if created:
    ...
    def create_carrier_with_fleet(self) -> User:
        ...
        staff = [
            ...
            ("piotr.kowalski@example.de", "piotr_kowalski", "Piotr Kowalski", "DL-6692031"),
        ]
        staff_profiles = []
        for email, username, full_name, license_number in staff:
            user, created = self._get_or_create_user(email, username, User.Role.DRIVER)
            if created:
    ...
    def create_accountant(self, carrier: User):
        user, created = self._get_or_create_user(
            "buchhaltung@example.de", "buchhaltung", User.Role.ACCOUNTANT
        )
        if created:
            AccountantProfile.objects.create(user=user, full_name="Julia Fischer")
